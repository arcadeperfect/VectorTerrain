...
namespace VectorTerrain.Scripts.Graph
{
	...
	public class TerrainGraph : NodeGraph {
		...
		public int GlobalSeed { get => _globalSeed; }
		...
		public Dictionary<string, SeedContainer> GetSeeds()
		{
			...
			Dictionary<string, SeedContainer> returnDict = new Dictionary<string, SeedContainer>();
			foreach (var node in nodes)
			{
		...
		public TerrainGraphOutput GetGraphOutput(TerrainGraphInput input, bool viz)
		{
			...
			bool backwards = (input.generation < 0); // if we are going backwards from the origin
			_globalSeed = Globals.GlobalSeed;
			
			if (backwards) _globalSeed += 123;
			...
			returnGraphOutput.totalDistanceAtEnd = (float) retrievedSectorData.TotalLengh + startDist;
