...
namespace VectorTerrain.Scripts.Nodes
{
    public abstract class TerrainNode : Node, IRealTimeUpdate, IPersistentSeed
    {
        ...
        [HideInInspector]
        [CanBeNull] public string ID = null;
        ...
        protected SeedContainer thisNodeSeedContainer = new();
        ...
        public void InitNoodles()
        {
            foreach (var fieldInfo in GetType().GetFields())
            {
                ...
                if (fieldInfo.FieldType == typeof(WeightNoodle))
                {
                    ...
                    if (noodle == null) throw new NullReferenceException("WeightNoodle is null, did you forget to initialise?");
                    
                    noodle.port = port;
                    noodle.plot = new(Plot.PlotType.Weight);
                ...
                if (fieldInfo.FieldType == typeof(SignalNoodle))
                {
                    ...
                    if (noodle == null) throw new NullReferenceException("SignalNoodle is null, did you forget to initialise?");
                    
                    noodle.port = port;
                    noodle.plot = new(Plot.PlotType.Signal);
                    Graph.graphPlotList.Add(noodle.plot);
                }
                ...
                if (fieldInfo.FieldType == typeof(FloatNoodle))
                {
                    ...
                    noodle.port = port;
                    noodle.plot = new(Plot.PlotType.Float);
                    Graph.graphPlotList.Add(noodle.plot);
                }
                ...
                if (fieldInfo.FieldType == typeof(MaskNoodle))
                {
                    ...
                    noodle.port = port;
                    noodle.plot = new(Plot.PlotType.Mask);
                    Graph.graphPlotList.Add(noodle.plot);
                }
            }
        }
        protected virtual void OnGenerationStart()
        {
            ...
            foreach (var signalNoodle in GetInputSignalNoodles())
            {
                signalNoodle.plot.Clear();
            }
        }
        ...
        private float GetSignal(NodePort port, Vector3 vectorSeed) 
        {
            ...
            var fieldInfo = this.GetType().GetField(port.fieldName);
            var signalNoodle = fieldInfo.GetValue(this) as SignalNoodle;
