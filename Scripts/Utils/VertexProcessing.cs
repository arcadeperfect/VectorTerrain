...
namespace VectorTerrain.Scripts.Utils
{
    public static class VertexProcessing
    {
        public static List<Vertex2> RemoveCollinearPoints(List<Vertex2> verts, float tolerance)
        {
            ...
            float prevSlope = Slope(verts[0], verts[1]);
            ...
            for (int i = 2; i < verts.Count; i++)
            {
                ...
                // seg.b = verts[i-1];
                // seg.a = verts[i];
                float slope = Slope(verts[i - 1], verts[i]); 
        ...
        public static List<VertexSegment> SegmentsFromVerts(List<Vertex2> verts)
        {
            ...
            {
                segs.Add(new VertexSegment(verts[i], verts[i+1]));
            }
        ...
        public static List<Vertex2> AverageVerts(List<Vertex2> verts, int width, float mix, int beginIndex, int endIndex, bool applyX, bool applyY)
        {
            if (width <= 0)
                return verts;

            List<Vertex2> newVerts = new List<Vertex2>();
            ...
            int iterationsY, float mixX, float mixY)
        {
            ...
            List<float> blurredX = new List<float>();
            List<float> blurredY = new List<float>();
        ...
        public static List<float> GaussianFilter1D(List<float> floats, int width, int iterations)
        {
            ...
            {
                floats = GaussianFilter1D(floats, width);
            }
            return floats;
        }
        public static List<float> GaussianFilter1D(List<float> floats, int width)
        {
            ...
            float avg = floats.Average();
        
        
        
            float sigma = Mathf.Sqrt(floats.Average(v => Mathf.Pow(v - avg, 2)));

        
            float[] kernel = GaussianKernal(width, sigma);
            float weightSum = kernel.Sum();
