...
namespace VectorTerrain.Scripts.Sector
{
    public class SectorController : MonoBehaviour
    {
        ...
        [SerializeField] GameObject endPositionObj;
        ...
        private static SectorController Process(TerrainGraphOutput graphOutput, Transform parent, VisualiserConfig viz)
        {
            ...
            GameObject newSector = new GameObject();
        ...
        public void Visualise(List<Plot> plots)
        {
            ...
            if (viz.DrawVerts)
            {
                ...
                foreach (var vert in sectorData.Verts)
                {
                    Gizmos.DrawWireSphere(vert.Pos, viz.VertsScale);
                }
            }
            ...
            if (viz.DrawPlots)
            {
                ...
                foreach (var plot in plots)
                {
                    ...
                    float yOffset = viz.plotYScale * 2;
                    yOffset *= c;
                    plot.ScaleXVals(Start.position.x, End.position.x);

                    for (int i = 1; i < plot.Xvals.Count; i++)
                    {
                    
                        float px = plot.Xvals[i - 1];
                        float py = plot.YVals[i - 1];
                        float x = plot.Xvals[i];
                        float y = plot.YVals[i];
        ...
        public void DestroyMe()
        {
            ...
            {
                if(Prefabs != null) foreach (var g in Prefabs) Destroy(g);
                Destroy(gameObject);
            }
            else
            {
                if(Prefabs != null) foreach (var g in Prefabs) DestroyImmediate(g);
